...
<body>
	...
	<div class="container">
		...
		<div class="row">
			...
			<c:if test="${not empty sessionScope.id }">
				<c:if test="${dto.likecheck != 1 }" >
				<form id="like" method="post" action="<c:url value="/BBS/Like.do"/>">
					<input type="hidden" name="no" value="${dto.no }"/>
					<input type="submit" value="좋아요"/>
				</form>
				</c:if>
				<c:if test="${dto.likecheck == 1 }">
				<form id="like" method="post" action="<c:url value="/BBS/unLike.do"/>">
					<input type="hidden" name="no" value="${dto.no }"/>
			...
			<c:if test="${dto.likecheck == 1 }">
				...
				<ul id="pillMenu" class="nav nav-pills center-block" style="width: 200px; margin-bottom: 10px">
					<c:if test="${sessionScope.id == dto.id}">
						<li><a href="<c:url value='/BBS/Edit.do?no=${dto.no}'/>" class="btn btn-success">수정</a></li>
						<li><a href="<c:url value='/BBS/Delete.do?no=${dto.no}'/>" class="btn btn-success">삭제</a></li>
					</c:if>
						<li><a href="<c:url value='/BBS/List.do'/>" class="btn btn-success">목록</a></li> 
				</ul>
			</div>
		</div>	​
	...
	<!-- 
		...
		$(".like").click(function(){
			if($(this).val()=="좋아요"){
				action="<c:url value="/BBS/Like.do"/>";
				type = "post";
			}
			else{
				action="<c:url value="/BBS/UnLike.do"/>";
				type = "post";
			}
